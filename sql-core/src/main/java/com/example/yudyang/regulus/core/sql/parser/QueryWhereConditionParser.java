...
public class QueryWhereConditionParser extends NeoBooleanExpParser implements QueryParser {
    @Override
    ...
    private void parseWhereCondition(ElasticDslContext dslContext, WhereClauseContext whereClauseContext) {
        if (whereClauseContext != null) {
            ...
            QueryBuilder queryBuilder = parseExpression(expressionContext);
            dslContext.getElasticSqlParseResult().setWhereCondition(queryBuilder);
            dslContext.getElasticSqlParseResult().getHighlighter().addAll(this.getHighlighters());
        }
    }
}
